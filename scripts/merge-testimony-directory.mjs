...
data.items = rows.map((row) => {
  ...
  return {
    ...
    author: row.author,
    family: row.author,
    excerpt: '文章已列入石牌四十週年家族信仰見證目錄，全文整理中。',
    content: '文章已列入石牌四十週年家族信仰見證目錄，請先由原文連結閱讀；全文與原始建立日期將於原貼文校對後補入。',
    status: '已建立目錄，全文待匯入',
    sourceUrl: row.sourceUrl,
    directoryOrder: row.directoryOrder,
  };
...
data.intro = `石牌教會四十週年家族信仰見證共 ${rows.length} 篇，可依標題、作者與內文搜尋。每篇保留原文連結，全文與原始日期依原貼文持續校對。`;
